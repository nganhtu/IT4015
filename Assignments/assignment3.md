# Bài tập lập trình 3: Cài đặt hệ mã dòng Trivium

*Sinh viên thực hiện: Nguyễn Anh Tú - 20184000*

Mã nguồn sử dụng để mã hóa file bằng Trivium:
...
def xor(a, b):  # xor 2 strings of binary of same length
    res = ""
...
def and_bit(a, b):  # and 2 strings of binary of same length
    res = ""
    for i in range(len(a)):
        if (a[i] == b[i] == "1"):
            res += "1"
...
Nội dung file `key.txt`:

```
2AC941AC16E189D8AF93

```

Độ phức tạp thuật toán là `O(n)` với `n` là độ dài bản rõ.

Bản rõ được em sử dụng để thống kê tốc độ mã hóa có 22360 ký tự. Thời gian mã hóa là khoảng 2.5 giây.

Khi Bob (giữ `IV`) cần giải mã, chỉ cần tạo ra `keystream` rồi coi bản mã như đầu vào để sử dụng Trivium. Khi đó đầu ra sẽ là bản rõ.

Bản rõ được sử dụng để test:
