...
def main():
    ...
    # Entering ciphertext
    read_ctbin = open(
        "introduction_to_information_security\\advanced_encryption_standard\\ctr\\ctbin.txt", "r")
    ...
    # Entering nonce
    read_nonce = open(
        "introduction_to_information_security\\advanced_encryption_standard\\ctr\\nonce.txt", "r")
    nonce = read_nonce.read().replace("\n", "")
    read_nonce.close()
    ...
    for i in range(0, len(ctbin), 128):
        ...
        while len(ctr_str) < 64:
            ctr_str = "0" + ctr_str
        ptblock = xor_str(AESEnc(nonce + ctr_str, key), ctblock)
        ptbin += ptblock
        ctr += 1
